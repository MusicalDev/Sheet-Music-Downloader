...
//             responseReceived = true; // Marcamos que llegó respuesta
...
export default function SheetMusicDownloader() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Remove these variables as they're creating confusion in the logic
        // Let the actual response events handle the states

        // Set up a network timeout
        const networkTimeout = setTimeout(() => {
            setErrorMessage(
                "No se pudo conectar con el servidor. Verifique su conexión a internet."
            );
        ...
        try {
            ...
            // Only set loading to false after successful download
            setIsLoading(false);
            setShowSuccess(true);
        } catch (error) {
            console.error("Error:", error);
